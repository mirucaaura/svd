...
class SVD:
    ...
    def armijo_rule(self, U, V, xi, eta, c):
        ...
        # (inner_product(gradF_x(U, V), c * xi) + inner_product(gradF_y(U, V), c * eta))
        return left >= right
    ...
    def cg(self, U0, V0, verbose=False):
        ...
        for i in range(self.itemax):
            ...
            # self.log_F.append(abs(F(U_next, V_next) - F(Ur, Vr)))
            # if norm_grad(xik_next, etak_next) < eps:
            #     break
            # set
            log_F_cg.append(self.F(Uk, Vk))
            Uk = U_next
            Vk = V_next
...
if __name__ == '__main__':
    ...
    Ur, _ = np.linalg.qr(np.random.rand(m, n))
    Vr, _ = np.linalg.qr(np.random.rand(n, n))
    ...
    U_ast, V_ast = svd.cg(U0, V0, verbose=True)

    plt.plot(abs(log_F_st - svd.F_opt(S, N)), color='firebrick', label='Steepest Descent')
    plt.plot(abs(log_F_cg - svd.F_opt(S, N)), color='navy', label='Conjugate Gradient Descent')
    plt.yscale('log')
    plt.grid(True)
    plt.legend()
    plt.xlabel('Iteration')
    plt.ylabel(r'$|f(U, V) - F_{\min}|$')
    plt.show()
